...
# One-command bring-up: build every host binary first so stack-up.sh never
# silently skips a service whose binary is missing (a fresh checkout works).
up: build-all stack-up
down: stack-down
stack-up:
	./scripts/stack-up.sh
stack-down:
...
build-agent-registry:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-agent-registry" && \
	  CGO_ENABLED=0 $(GO) build -o bin/cofiswarm-agent-registry ./cmd/cofiswarm-agent-registry
build-slot-manager:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-slot-manager" && \
	  CGO_ENABLED=0 $(GO) build -o bin/cofiswarm-slot-manager ./cmd/cofiswarm-slot-manager
build-kvpool:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-kvpool" && \
	  CGO_ENABLED=0 $(GO) build -o bin/cofiswarm-kvpool ./cmd/cofiswarm-kvpool
# zmq-bridge uses go-zeromq/zmq4 (pure Go) — CGO off matches its own Dockerfile.
build-zmq-bridge:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-zmq-bridge" && \
	  CGO_ENABLED=0 $(GO) build -o bin/cofiswarm-zmq-bridge ./cmd/cofiswarm-zmq-bridge
build-convert:
...
build-rag-worker:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-rag-worker" && \
	  CGO_ENABLED=0 $(GO) build -o bin/cofiswarm-rag-worker ./cmd/cofiswarm-rag-worker
# rag needs CGO for the sqlite-vec extension; CC=cc avoids a stale mlx-env CC override.
build-rag:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-rag" && \
	  CGO_ENABLED=1 CC=cc $(GO) build -o bin/cofiswarm-rag ./cmd/cofiswarm-rag
build-config:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-config" && \
	  CGO_ENABLED=0 $(GO) build -o bin/cofiswarm-config ./cmd/cofiswarm-config
build-orchestrate:
	cd "$(or $(COFISWARM_REPOS_ROOT),$(HOME)/cofiswarm/repos)/cofiswarm-orchestrate" && \
	  CGO_ENABLED=0 $(GO) build -o bin/orch-sidecar ./cmd/orch-sidecar
build-sidecars: build-convert build-rag-worker build-rag build-config build-orchestrate
# Every host binary that scripts/stack-up.sh starts. `make up` runs this first.
build-all: build-dispatch build-modes build-configure build-observer \
	build-agent-registry build-slot-manager build-kvpool build-zmq-bridge \
	build-sidecars
test-scale0-gate:
...
test-migration-complete-signoff-gate:
	./test/scripts/test-migration-complete-signoff-gate.sh
render-migration-complete-signoff:
	MIGRATION_COMPLETE_SKIP_GATE=1 ./test/scripts/render-migration-complete-signoff.sh
migration-complete: release-cut remote-push post-migration render-migration-complete-signoff
test-remote-complete-signoff-gate:
	./test/scripts/test-remote-complete-signoff-gate.sh
render-remote-complete-signoff:
	REMOTE_COMPLETE_SKIP_GATE=1 ./test/scripts/render-remote-complete-signoff.sh
remote-complete: test-remote-complete-signoff-gate render-remote-complete-signoff
test-migration-handoff-signoff-gate:
	./test/scripts/test-migration-handoff-signoff-gate.sh
render-migration-handoff-signoff:
	MIGRATION_HANDOFF_SKIP_GATE=1 ./test/scripts/render-migration-handoff-signoff.sh
migration-handoff: remote-complete test-migration-handoff-signoff-gate render-migration-handoff-signoff
verify-remote-push:
